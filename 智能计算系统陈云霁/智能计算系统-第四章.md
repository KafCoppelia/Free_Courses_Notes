...
2. 梯度裁剪(Gradient Clipping)
	- 示例：对梯度的$L^2$范式进行裁剪：
	梯度的$L^2$范式为：$\parallel t\parallel_2=\sqrt{grad(w_1)^2+grade(w_2)^2+\cdots}$
	设置裁剪阈值$c$，则当$\parallel t\parallel_2>c$时，$grade(w_i)=\dfrac{c}{\parallel t\parallel_2}\cdot grad(w_i)$；当$\parallel t\parallel_2\le c$时，$grade(w_i)$不变；
