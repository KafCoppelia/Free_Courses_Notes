...
### 总体架构
![Architecture_of_DLP-M](./Images/7.2-24-Architecture_of_DLP-M.png)
- 多核处理器分层结构设计
	- 一个DLP-M由多个DLP-C构成；
	- 一个DLP-C由多个DLP-S构成；
	- 减少NoC的负载核开销；
...
### Cluster架构
- MEMCORE
	![Architecture_of_MEMCORE](./Images/7.2-25-Architecture_of_MEMCORE.png)
	- 共享存储模块SMEM(Shared Memory)；
	- 广播总线(Broadcast Bus)；
	- Cluster直接内存访问CDMA(Cluster Direct Memory Access)；
	- 全局直接内存访问GDMA(Global Direct Memory Access)；
- 广播总线；
	![Methods_of_Broadcast](./Images/7.2-26-Methods_of_Broadcast.png)
	- 深度学习数据的复用特性；
	- 读写请求：单播写、单播读，多播；
- 多播的应用：大规模卷积运算
...
- 多核同步模型
	...
	- 双核协同指令流
	![Command_Flow_Between_Cores](./Images/7.2-28-Command_Flow_Between_Cores.png)
...
- 核间互联拓扑结构
	...
	![Topological_Structure_of_Multi-cores](./Images/7.2-29-Topological_Structure_of_Multi-cores.png)

- 互联方式
	- 总线互联
		![Bus_Connection](./Images/7.2-30-Bus_Connection.png)
		- 公共数据干线
		- 拓展性差
	- 片上网络
		![NOC](./Images/7.2-31-NOC.png)
		- 片上互联
		- 性能和功耗有优势
		- 拓展性好
- DLP-C互联

	![Interconnect_of_DLP-C](./Images/7.2-32-Interconnect_of_DLP-C.png)
