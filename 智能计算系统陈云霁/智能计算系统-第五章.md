...
- 符号求导法：直接对代数表达式求解，最后才代入数字
  ...
  $$
  frac{d}{dx}\left(f(x)+g(x)\right)=\dfrac{d}{dx}f(x)+\dfrac{d}{dx}g(x)$$$$\dfrac{d}{dx}f(x)g(x)=\left(\dfrac{d}{dx}f(x)\right)g(x)+f(x)\left(\dfrac{d}{dx}g(x)\right)$$$$\dfrac{d}{dx}\dfrac{f(x)}{g(x)}=\dfrac{f'(x)g(x)-f(x)g'(x)}{g(x)^2}
  $$
...
#### 计算图优化
![Optimization_of_Calc_Flow](5.2-13-Optimization_of_Calc_Flow.png)
- TF的图优化由Grappler模块实现；
...
![Architecture_of_TF](./Images/5.2-17-Architecture_of_TF.png)
...
#### 执行器逻辑
![Stream_of_Executor](./Images/5.2-18-Stream_of_Executor.png)
- 执行流(Stream)：一个能存储计算任务的队列；
- 流间任务可以并行执行，流内任务串行执行；

ScheduleReady逻辑流程

  ![Schedule_Ready](./Images/5.2-19-Schedule_Ready.png)
...
#### 算子实现
![OpKernel](./Images/5.2-20-OpKernel.png)
...
1. 构建VGG网络
2. 内部构图逻辑
![Inner_Logic_of_Construct](./Images/5.2-21-Inner_Logic_of_Construct.png)
4. 训练模型
5. 加载模型执行预测
6. SessionRun由Python到C/C++ API
![Python_to_cpp_API](./Images/5.2-22-Python_to_cpp_API.png)
7. C++中的SessionRun流程
![SessionRun_in_cpp](./Images/5.2-23-SessionRun_in_cpp.png)
